...
echo "HTTPS Proxy Upgrade Script for macOS"
echo "======================================"
...
if [ ! -d "$INSTALL_DIR" ]; then
  ...
  echo "Please run install_macos.sh for a fresh installation."
  exit 1
fi
...
# ── Step 2: Update launchd service ──
echo ""
echo "=== Step 2: Updating launchd service ==="
if [ -f "deploy/launchd/com.proxy.https.plist" ]; then
  cp -f deploy/launchd/com.proxy.https.plist "$PLIST_PATH"
  chmod 644 "$PLIST_PATH"
  echo "Launchd service updated."
else
  echo "Warning: deploy/launchd/com.proxy.https.plist not found, skipping."
fi
...
if [ ! -f "$DATA_DIR/GeoLite2-Country.mmdb" ]; then
  ...
  echo "  rm $DATA_DIR/GeoLite2-Country.mmdb && sudo ./deploy/upgrade_macos.sh"
fi
...
echo "╠═══════════════════════════════════════════════════════╣"
echo "║  Check status: launchctl list | grep com.proxy.https  ║"
echo "║  View logs:    cat /var/log/https-proxy/output.log    ║"
